...
export async function POST(req: Request) {
  ...
  const uidHost =
    process.env.BOOKING_ICS_UID_HOST?.trim() || "bookings.mentemaestra.space";
  ...
  try {
    googleCalendarUrl = buildGoogleCalendarTemplateUrl({
      ...
      details: [
        ...
        "",
        "Reserva vía mentemaestra.space",
      ]
        .filter(Boolean)
        .join("\n"),
      dateYmd: date,
  ...
  try {
    ...
    const descParts = [
      ...
      "",
      "Booked via mentemaestra.space — add to your calendar from this invite.",
    ].filter(Boolean);
