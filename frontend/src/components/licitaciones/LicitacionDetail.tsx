...
function pickNumber(o: unknown, path: string[]): number | undefined {
  let cur: unknown = o;
  for (const k of path) {
    if (!cur || typeof cur !== "object") return undefined;
    cur = (cur as Record<string, unknown>)[k];
...
function getAny(o: unknown, keys: string[]): unknown | undefined {
...
function extractDetalle(payload: unknown) {
  const root = isRecord(payload) ? payload : null;
  const listado = root && Array.isArray(root["Listado"]) ? root["Listado"] : null;
  const first = listado?.[0];
  const firstRec = isRecord(first) ? first : null;
  const licNested = firstRec?.["Licitacion"];
  const lic = isRecord(licNested) ? licNested : firstRec;
  const buyerRaw = lic?.["Comprador"];
  const buyer = isRecord(buyerRaw) ? buyerRaw : null;
  const fechasRaw = lic?.["Fechas"];
  const fechas = isRecord(fechasRaw) ? fechasRaw : null;
  const itemsWrap = lic?.["Items"];
  const itemsRec = isRecord(itemsWrap) ? itemsWrap : null;
  const items = Array.isArray(itemsRec?.["Listado"]) ? itemsRec["Listado"] : null;
...
export default function LicitacionDetail({ codigo }: { codigo: string }) {
  ...
  const ctx = useMemo(() => extractDetalle(payload), [payload]);
