 ...
 *
 * • `DATABASE_URL` — Supabase Postgres connection string (required; apply `backend/migrations/*.sql` in order). Local dev and production both use the **transaction pooler** URI (port **6543**) with `?pgbouncer=true&sslmode=require`; `src/lib/db.ts` patches common omissions for `pooler.supabase.com`.
 ...
 * • `BOOKING_PUBLIC_BASE_URL` — public site origin for links in emails and signed `.ics` downloads (no trailing slash), e.g. `https://mentemaestra.studio`. Legacy `mentemaestra.space` in this value is rewritten to `.studio`. If unset: Vercel **production** uses `https://mentemaestra.studio`; preview uses `https://$VERCEL_URL`; local dev uses `http://localhost:3000`.
 ...
 * A **multi-day or all-day** event that spans your booking range marks **every** 15-minute slot in that range as busy (same as being fully booked). Use `npm run booking:availability` from the monorepo root with the dev server running, or `node scripts/print-booking-availability.mjs --debug` from `frontend/` (non-production) to inspect `largestIntervalHours` and interval samples if the grid is unexpectedly empty.
 */
